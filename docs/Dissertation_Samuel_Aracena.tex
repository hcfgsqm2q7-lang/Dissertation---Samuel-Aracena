...
\hline
\textbf{Declared Word Count:} & \multicolumn{9}{p{10.76cm}|}{8733} \\
\hline
\end{tabular}
\endgroup
...
Reporting counts (how many reports mention a district in a given month) are modelled using negative binomial regression. This is appropriate for count data because a linear regression does not naturally respect the non-negative, discrete nature of counts, while a simpler Poisson model assumes the conditional mean and variance are equal and can therefore be inappropriate when the data are overdispersed, meaning the variance in report counts is larger than a Poisson model assumes. The main model takes the form:
\[
...
As an additional analysis of reporting coverage, complementing the comparisons above rather than directly answering RQ1's question about source characteristics, a logistic regression was used to examine whether basic reporting coverage, whether a district was mentioned at all in a given month, could be predicted from conflict, market coverage, climate and regional and month controls (Section 3.5). The full model reached an in-sample AUC of 0.775, but its out-of-sample AUC, tested on districts held out entirely, fell to 0.640. Removing region and climate, which together contribute almost nothing once conflict and market are already in the model (chi-squared = 0.23, p = 0.89, for climate specifically), produced a lower in-sample AUC (0.739) but a higher out-of-sample AUC of 0.715, meaning regional information contributed to apparent in-sample fit while actually reducing how well the model generalised to districts it had not seen. Full diagnostics for both versions are in Appendix D. This is a second, independent line of evidence that district-level identity can look more informative than it really is once tested on unseen data.
...
One of the main findings is that reporting volume is strongly associated with recorded conflict, but shows no comparable association with vegetation stress. In the main model (Section 3.5), a one standard deviation increase in logged conflict counts is associated with an incidence rate ratio of 1.69 (95\% CI [1.39, 2.06], $p < 0.0001$), so reports increase by approximately 69\% per standard deviation, holding month and vegetation stress fixed. Vegetation stress is not significantly associated with reporting volume in this pooled specification: an IRR of 0.998 (95\% CI [0.78, 1.27], $p = 0.98$), indistinguishable from no relationship. A Wald test confirms the two coefficients differ significantly in magnitude (chi-squared $= 9.00$, $p = 0.0027$). The choice of negative binomial over Poisson is supported by the fitted overdispersion parameter (alpha $= 1.333$, $p < 0.0001$), which is well above zero, so the variance in report counts exceeds what Poisson would assume.
...
Collection mechanism & Media and monitoring-partner coding (65.3\% cite a local partner) & Satellite imagery, algorithmic, no ground survey required & Weekly market surveys by WFP enumerators & Self-submitted situation reports from humanitarian orgs \\
...
\midrule
AUC & 0.775 & 0.640 \\
Accuracy & 0.721 & 0.640 \\
Precision (of predicted yes) & 0.743 & 0.686 \\
Recall / sensitivity & 0.814 & 0.732 \\
Specificity & 0.585 & 0.506 \\
F1 & 0.777 & 0.708 \\
Brier score (lower = better) & 0.186 & 0.241 \\
...
\noindent Out-of-sample confusion matrix (full model): of districts truly not mentioned, 179 were correctly predicted no and 175 incorrectly predicted yes; of districts truly mentioned, 140 were incorrectly predicted no and 382 correctly predicted yes.
...
\midrule
AUC & 0.739 & 0.715 \\
Accuracy & 0.685 & 0.664 \\
Precision (of predicted yes) & 0.717 & 0.701 \\
Recall / sensitivity & 0.778 & 0.762 \\
Specificity & 0.548 & 0.520 \\
F1 & 0.746 & 0.730 \\
Brier score (lower = better) & 0.201 & 0.211 \\
McFadden pseudo R\textsuperscript{2} & 0.132 & --- \\
\bottomrule
\end{tabular}
\caption{Parsimonious model diagnostics (region and climate removed).}
\end{table}

\noindent Out-of-sample confusion matrix (parsimonious model): of districts truly not mentioned, 184 were correctly predicted no and 170 incorrectly predicted yes; of districts truly mentioned, 124 were incorrectly predicted no and 398 correctly predicted yes.

\textit{The parsimonious model, without region or climate, generalises better out-of-sample (AUC 0.715 versus 0.640) despite a lower in-sample AUC, which is why Section 3.5 and Section 4 treat region's apparent explanatory power with caution.}
...
\midrule
\endhead
{\ttfamily o}\quad \textsuperscript{H}ANIMALS or animal parts. & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{M}CELL LINES (established and commercially available cells, biological research). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{H}CELL CULTURE (Primary: from animal/human cells, biological research). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{H}CLINICAL Audits or Assessments (e.g.\ in medical settings). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}CONFLICT of INTEREST or lack of IMPARTIALITY. If unsure see ``Code of Practice for Research'' (Sec 3.5) at: \url{https://unihub.mdx.ac.uk/study/spotlights/types/research-at-middlesex/research-ethics} & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}DATA to be used that is not freely available (e.g. secondary data needing permission for access or use). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}DAMAGE (e.g., to precious artefacts or to the environment) or present a significant risk to society). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}EXTERNAL ORGANISATION, research carried out within an external organisation or your research is commissioned by a government (or government body). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{M}FIELDWORK (e.g., biological research, ethnography studies). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{H}GENETICALLTY MODIFIED ORGANISMS (GMOs) (biological research). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{H}GENE THERAPY including DNA sequenced data (biological research). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{M}HUMAN PARTICIPANTS, ANONYMOUS Questionnaires (participants not identified or identifiable). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}HUMAN PARTICIPANTS, IDENTIFIABLE (participants are identified or can be identified): survey questionnaire/ INTERVIEWS / focus groups / experiments / observation studies/ evaluation studies. & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{H}HUMAN TISSUE (e.g., human relevant material, e.g., blood, saliva, urine, breast milk, faecal material). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{H}ILLEGAL/HARMFUL activities research (e.g., development of technology intended to be used in an illegal/harmful context or to breach security systems, searching the internet for information on highly sensitive topics such as child and extreme pornography, terrorism, use of the DARK WEB, research harmful to national security). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}PERMISSION is required to access premises or research participants. & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}PERSONAL DATA PROCESSING (Any activity with data that can directly or indirectly identify a living person). For example, data gathered from interviews, databases, digital devices such as mobile phones, social media or internet platforms or apps with or without individuals'/owners' knowledge or consent, and/or could lead to individuals/owners being IDENTIFIED or SPECIAL CATEGORY DATA (GDPR\textsuperscript{1}) or CRIMINAL OFFENCE DATA. \textsuperscript{1}Special category data (GDPR, Art.9): ``personal data revealing racial or ethnic origin, political opinions, religious or philosophical beliefs, or trade union membership, and the processing of genetic data, biometric data for the purpose of uniquely identifying a natural person, data concerning health or data concerning a natural person's sex life or sexual orientation''. & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}PUBLIC WORKS DOCTORATES: Evidence of permission is required for use of works/artifacts (that are protected by Intellectual Property (IP) Rights, e.g. copyright, design right) in a doctoral critical commentary when the IP in the work/artifacts jointly prepared/produced or is owned by another body & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{H}RISK OF PHYSICAL OR PSYCHOLOGICAL HARM (e.g., TRAVEL to dangerous places in your own country or in a foreign country, research with NGOs/humanitarian groups in conflict/dangerous zones, development of technology/agent/chemical that may be harmful to others, any other foreseeable dangerous risks). & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}SECURITY CLEARANCE, required for research. & $\Box$ Yes & $\boxtimes$ No \\
{\ttfamily o}\quad \textsuperscript{X}SENSITIVE TOPICS (e.g., anything deeply personal and distressing, taboo, intrusive, stigmatising, sexual in nature, potentially dangerous, etc). & $\Box$ Yes & $\boxtimes$ No \\
\bottomrule
